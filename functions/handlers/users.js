...
exports.getUserProfile = (req, res) => {
  ...
  db.doc(`/users/${req.user.handel}`)
    ...
    .then((data) => {
      userProfile.matches = [];
      data.forEach((doc) => {
        userProfile.matches.push(doc.data());
      });
      return (
        db
          .collection("notifications")
          .where("getter", "==", req.user.handel)
...
exports.getOtherUsers = (req, res) => {
  let userData = {};
  db.doc(`/users/${req.params.handel}`)
    .get()
    .then((doc) => {
      if (doc.exists) {
        ...
        return (
          db
            ...
            .where("userName", "==", req.params.handel)
            //.orderBy("createdAt", "desc")
            .get()
        );
      } else {
...
exports.setNotoficationsRead = (req, res) => {
  let batch = db.batch();
